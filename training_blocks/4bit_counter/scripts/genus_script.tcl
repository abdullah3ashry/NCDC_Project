...
if {$FLOW_TYPE == "physical"} {
    ...
    write_do_lec -revised_design fv_map -logfile ${RPT_DIR}/lec_rtl_to_mapped.log > ${OUT_DIR}/rtl_to_mapped0.1pF.do 
    
    # AUTOMATED FIX 1: Prevent Conformal from closing automatically
    exec sed -i "/exit -f/d" ${OUT_DIR}/rtl_to_mapped0.1pF.do
    
    # syn_opt: Final gate-level optimization using real RC parasitics
    syn_opt
    
    # --- LEC CHECK 2: Mapped Netlist vs. Optimized Netlist ---
    write_do_lec -golden_design fv_map -revised_design fv_opt -logfile ${RPT_DIR}/lec_mapped_to_opt.log > ${OUT_DIR}/mapped_to_opt0.1pF.do 
    
    # AUTOMATED FIX 2: Patch the fv_opt file path bug AND prevent Conformal from closing
    exec sed -i "/read_design/s| fv_opt| ${NETLIST_DIR}/${DESIGN}_${FLOW_TYPE}_synth.v|g" ${OUT_DIR}/mapped_to_opt0.1pF.do
    exec sed -i "/exit -f/d" ${OUT_DIR}/mapped_to_opt0.1pF.do
...
# ==============================================================================
# 5. DFT SCAN STITCHING (POST-SYNTHESIS)
# ==============================================================================
if {$INCLUDE_DFT == "true"} {
...
report_timing -full_pin_names  -max_paths 10 > ${RPT_DIR}/${DESIGN}_${FLOW_TYPE}_timing0.1pF.rpt

# report_area: Summarizes the physical footprint (cell area) of the design.
report_area > ${RPT_DIR}/${DESIGN}_${FLOW_TYPE}_area0.1pF.rpt

# report_gates: Provides a breakdown of the standard cell types used (e.g., NAND, NOR, DFF).
report_gates > ${RPT_DIR}/${DESIGN}_${FLOW_TYPE}_gates0.1pF.rpt

# report_power: Estimates the dynamic, leakage, and total power consumption.
report_power > ${RPT_DIR}/${DESIGN}_${FLOW_TYPE}_power0.1pF.rpt
...
report_qor -levels_of_logic > ${RPT_DIR}/${DESIGN}_${FLOW_TYPE}_qor0.1pF.rpt
...
if {$INCLUDE_DFT == "true"} {
    ...
    report_scan_chains > ${RPT_DIR}/${DESIGN}_${FLOW_TYPE}_scan_chains0.1pF.rpt
}

# ------------------------------------------------------------------------------
# 7. DATA EXPORT (HANDOFF)
# ------------------------------------------------------------------------------
...
write_hdl $DESIGN -mapped > ${NETLIST_DIR}/${DESIGN}_${FLOW_TYPE}_synth0.1pF.v

# write_sdf: Exports timing delays (Setup/Hold/IOPATH) for Gate-Level Simulation.
write_sdf -setuphold merge_always -recrem merge_always -design $DESIGN > ${OUT_DIR}/${DESIGN}_${FLOW_TYPE}_synth0.1pF.sdf

if {$FLOW_TYPE == "physical"} {
    # write_db: Saves the entire design state to be opened directly in Innovus.
    write_db ${DB_DIR}/${DESIGN}_synth0.1pF.db
